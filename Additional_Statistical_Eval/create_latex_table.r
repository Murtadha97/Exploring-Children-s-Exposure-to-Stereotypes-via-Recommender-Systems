...
root_dir <- "C:/Users/rungruh/OneDrive - Delft University of Technology/Documents/Projects/RecSys2024 - StereotypesRecSysChildren/Exploring-Children-s-Exposure-to-Stereotypes-via-Recommender-Systems/Results/SDM"
...
# Comparing with Random and MostPop
...
for (file in csv_files) {
  ...
  file_name <- basename(file)
    ...
    # Check if data includes the column "VSM"
    if (!"VSM" %in% colnames(data)) {
      # Add the column "VSM" and fill with NaNs
      data$VSM <- 0
    }
  ...
  # 25 if file_name includes "Goodreads"
  if (grepl("Goodreads", file_name)) {
    num_comparisons = 25
  } else {
    num_comparisons = 27
  }
...
algorithm_means_goodreads <- algorithm_means_goodreads %>%
  ...
  mutate(metric = ifelse(grepl("Goodreads_chatgpt_gender_HITS.csv", metric), 
  "HIT$_{GPT, Gender}$", metric)) %>%
  mutate(metric = ifelse(grepl("Goodreads_chatgpt_gender_MRRBAD.csv", metric), 
  "MRR$_{GPT, Gender}$", metric)) %>%
  mutate(metric = ifelse(grepl("Goodreads_chatgpt_gender_REC-ST.csv", metric), 
  "REC-ST$_{GPT, Gender}$", metric)) %>%
  mutate(metric = ifelse(grepl("Goodreads_chatgpt_race_HITS.csv", metric), 
  "HIT$_{GPT, Race}$", metric)) %>%
  mutate(metric = ifelse(grepl("Goodreads_chatgpt_race_MRRBAD.csv", metric), 
  "MRR$_{GPT, Race}$", metric)) %>%
  mutate(metric = ifelse(grepl("Goodreads_chatgpt_race_REC-ST.csv", metric), 
  "REC-ST$_{GPT, Race}$", metric)) %>%
  mutate(metric = ifelse(grepl("Goodreads_chatgpt_religion_HITS.csv", metric), 
  "HIT$_{GPT, Religion}$", metric)) %>%
  mutate(metric = ifelse(grepl("Goodreads_chatgpt_religion_MRRBAD.csv", metric), 
  "MRR$_{GPT, Religion}$", metric)) %>%
  mutate(metric = ifelse(grepl("Goodreads_chatgpt_religion_REC-ST.csv", metric), 
...
algorithm_means_ml <- algorithm_means_ml %>%
  select(metric, Random, MostPop, ItemKNN, UserKNN, BPRMF, FunkSVD, MF, PMF, PureSVD,
         Slim, DeepFM, NeuMF, AMF, MultiVAE, VSM) %>%
  ...
  "REC-ST$_{BiasMeter, Religion}$", metric)) %>%
  mutate(metric = ifelse(grepl("ML-children_chatgpt_gender_HITS.csv", metric), 
  "HIT$_{GPT, Gender}$", metric)) %>%
  mutate(metric = ifelse(grepl("ML-children_chatgpt_gender_MRRBAD.csv", metric), 
  "MRR$_{GPT, Gender}$", metric)) %>%
  mutate(metric = ifelse(grepl("ML-children_chatgpt_gender_REC-ST.csv", metric), 
  "REC-ST$_{GPT, Gender}$", metric)) %>%
  mutate(metric = ifelse(grepl("ML-children_chatgpt_race_HITS.csv", metric), 
  "HIT$_{GPT, Race}$", metric)) %>%
  mutate(metric = ifelse(grepl("ML-children_chatgpt_race_MRRBAD.csv", metric), 
  "MRR$_{GPT, Race}$", metric)) %>%
  mutate(metric = ifelse(grepl("ML-children_chatgpt_race_REC-ST.csv", metric), 
  "REC-ST$_{GPT, Race}$", metric)) %>%
  mutate(metric = ifelse(grepl("ML-children_chatgpt_religion_HITS.csv", metric), 
  "HIT$_{GPT, Religion}$", metric)) %>%
  mutate(metric = ifelse(grepl("ML-children_chatgpt_religion_MRRBAD.csv", metric), 
  "MRR$_{GPT, Religion}$", metric)) %>%
  mutate(metric = ifelse(grepl("ML-children_chatgpt_religion_REC-ST.csv", metric), 
  "REC-ST$_{GPT, Religion}$", metric)) %>%
